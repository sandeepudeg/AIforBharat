...
class BedrockKnowledgeBase:
    ...
    def create_knowledge_base(
        ...
        try:
            ...
            kb_obj = response.get("knowledgeBase", {})
            kb_id = kb_obj.get("knowledgeBaseId")    # ✅ correct key
            kb_status = kb_obj.get("status")
    ...
    #     # ---- Base payload (what create_data_source API expects) ------------
    ...
    #     data_source_cfg: Dict[str, Any] = {"type": data_source_type}

    #     if data_source_type == "S3":
    #         # We expect data_source_config to be at least {"bucket_name": "..."}
    #         bucket_name = data_source_config["bucket_name"]
    #         # inclusion_prefixes = data_source_config.get("inclusion_prefixes", [""])
    #         inclusion_prefixes = data_source_config.get("inclusion_prefixes", [""])
    #         s3_cfg: Dict[str, Any] = {"bucketArn": f"arn:aws:s3:::{bucket_name}",}
    #         if inclusion_prefixes:
    #             s3_cfg["inclusionPrefixes"] = inclusion_prefixes
    #         data_source_cfg["s3Configuration"] = s3_cfg

    #     elif data_source_type == "WEB":
    #         data_source_cfg["webConfiguration"] = data_source_config

    #     # (Extend here for CONFLUENCE / SHAREPOINT / SALESFORCE if needed.)

    #     ds_config["dataSourceConfiguration"] = data_source_cfg
    ...
    #         return {
    #             "data_source_id": ds.get("dataSourceId") or ds.get("id"),
    ...
    def create_data_source(
        ...
        if data_source_type == "S3":
            ...
            # Only set inclusionPrefixes if we have non-empty strings
            if inclusion_prefixes is not None:
                cleaned = [p for p in inclusion_prefixes if p]
                if not cleaned:
                    raise ValueError(
                        "inclusion_prefixes must contain at least one non-empty string"
                    )
                s3_cfg["inclusionPrefixes"] = cleaned
        ...
        # DEBUG: print the final payload once to verify
        # import json
        # print("DEBUG ds_config:", json.dumps(ds_config, indent=2, default=str))
    ...
    def _get_embedding_dimensions(self, model_id: str) -> int:
        ...
        raise ValueError(f"Unknown embedding model for dimension lookup: {model_id}")
