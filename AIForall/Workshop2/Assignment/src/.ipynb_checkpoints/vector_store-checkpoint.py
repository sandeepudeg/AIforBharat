...
class VectorIndexManager:
    ...
    def __init__(self, aws_config: AWSConfig):
        ...
        print("VectorIndexManager region from AWSConfig:", self.region)
        try:
            print("oss_client region:", self.oss_client.meta.region_name)
        except Exception:
            pass
    ...
    def create_vector_index(
        ...
        # Map friendly similarity names to OpenSearch space_type
        if similarity_metric == "cosine":
            space_type = "cosinesimil"
        elif similarity_metric == "euclidean":
           space_type = "l2"
        else:
        # assume caller passed a valid OpenSearch value like "innerproduct"
        ...
        try:
            ...
            # 5. If index exists, return already_exists
    ...
    def _get_collection_endpoint(self, collection_name: str) -> str:
        ...
        """
        try:
            # DEBUG: see what collections THIS client can see
            debug_list = self.oss_client.list_collections(maxResults=50)
            print("Collections visible to VectorIndexManager:")
            for c in debug_list.get("collectionSummaries", []):
                print("  -", c.get("name")) # Debug code
            response = self.oss_client.batch_get_collection(
                names=[collection_name]
            )
            print("batch_get_collection response:", response) # Debug code
