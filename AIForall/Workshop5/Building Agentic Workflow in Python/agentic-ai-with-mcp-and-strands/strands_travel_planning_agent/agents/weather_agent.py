...
Uses OpenWeatherMap API (free tier) for real weather data.
"""
...
analyze it, and provide comprehensive recommendations."""
    ...
    def get_forecast(self, destination: str, start_date: str, end_date: str) -> dict:
        ...
        try:
            ...
            forecast = self._get_openweather_forecast(destination, start_date, end_date)
            if forecast:
                return forecast
        ...
        return self._get_mock_forecast(destination, start_date, end_date)
    
    def _get_openweather_forecast(self, destination: str, start_date: str, end_date: str) -> dict:
        """Get forecast from OpenWeatherMap API (free tier - 5 day forecast)."""
        try:
            ...
            for forecast in data.get("list", [])[:40]:  # 5 days * 8 forecasts per day
                dt = datetime.fromtimestamp(forecast.get("dt", 0))
                
                day_forecast = {
                    "date": dt.strftime("%Y-%m-%d"),
                    "day_of_week": dt.strftime("%A"),
                    "high_temp": forecast.get("main", {}).get("temp_max", 20),
                    "low_temp": forecast.get("main", {}).get("temp_min", 12),
    ...
    def _get_mock_forecast(self, destination: str, start_date: str, end_date: str) -> dict:
        ...
        while current <= end:
            day_forecast = {
                ...
                "high_temp": 20 + (hash(current.isoformat()) % 15),
                "low_temp": 12 + (hash(current.isoformat()) % 10),
