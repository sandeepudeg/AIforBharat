...
from strands import Agent, tool
from strands_tools import file_read, file_write, editor # Removed python_repl, shell
import json
...
Note: Direct code execution (REPL) and Shell access are disabled on this Windows environment. Please provide code snippets that the user can run manually.
"""

@tool
def computer_science_assistant(query: str) -> str:
    """
    Process computer science queries. (Windows Compatible Version)
    """
    formatted_query = f"Please address this computer science or programming question: {query}"
    
    try:
        print("Routed to Computer Science Assistant")
        # Tool list updated for Windows compatibility
        cs_agent = Agent(
            system_prompt=COMPUTER_SCIENCE_ASSISTANT_SYSTEM_PROMPT,
